...
public final class AdvancedGeneratorMenu extends AbstractContainerMenu {
    ...
    public AdvancedGeneratorMenu(int containerId, Inventory inventory) {
        this(containerId, inventory, new SimpleContainer(MACHINE_SLOTS), new SimpleContainerData(DATA_COUNT));
    }

    public AdvancedGeneratorMenu(int containerId, Inventory inventory, AdvancedGeneratorBlockEntity generator) {
        this(containerId, inventory, createMachineContainer(generator), createData(generator));
    }

    private AdvancedGeneratorMenu(int containerId, Inventory inventory, Container machine, ContainerData data) {
        super(ICEGRegistries.ADVANCED_GENERATOR_MENU.get(), containerId);
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        } else if (slots.get(FUEL_SLOT).mayPlace(stack)) {
    ...
    private static Container createMachineContainer(AdvancedGeneratorBlockEntity generator) {
        return new Container() {
            ...
            public boolean isEmpty() {
                return generator.getFuel().isEmpty() && generator.getChargeBattery().isEmpty();
            }

            @Override
            public ItemStack getItem(int slot) {
                return switch (slot) {
                    case FUEL_SLOT -> generator.getFuel();
                    case BATTERY_SLOT -> generator.getChargeBattery();
            ...
            public void setItem(int slot, ItemStack stack) {
                switch (slot) {
                    case FUEL_SLOT -> generator.setFuel(stack);
                    case BATTERY_SLOT -> generator.setChargeBattery(stack.isEmpty() ? ItemStack.EMPTY : stack.copyWithCount(1));
                    default -> {
                        return;
                    }
                }
            ...
            public boolean canPlaceItem(int slot, ItemStack stack) {
                return switch (slot) {
                    case FUEL_SLOT -> generator.isFuel(stack);
                    case BATTERY_SLOT -> generator.isChargeBattery(stack);
                    default -> false;
                };
            }
            ...
            public void clearContent() {
                generator.setFuel(ItemStack.EMPTY);
    ...
    private static ContainerData createData(AdvancedGeneratorBlockEntity generator) {
        return new ContainerData() {
            ...
            public int get(int index) {
                return switch (index) {
                    ...
                    case 2 -> generator.getFuelTicks();
                    case 3 -> generator.getMaxFuelTicks();
                    default -> 0;
                };
            }
